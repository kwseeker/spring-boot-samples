...
public class ThreadPoolWrapper {
    ...
    public void shutdown() {
        System.out.println("threadPool2 shutdown ..." + System.currentTimeMillis());
        this.threadPool.shutdown();
        try {
            boolean terminated = this.threadPool.awaitTermination(WAIT_TERMINATED, TimeUnit.SECONDS);
            if (!terminated) {
                List<Runnable> undoneTasks = this.threadPool.shutdownNow();
                //... other handle
                System.out.println("undoneTasks size: " + undoneTasks.size());
            }
            System.out.println("threadPool2 terminated ..." + System.currentTimeMillis());
